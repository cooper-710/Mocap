#!/usr/bin/env python3
"""
Motion Capture CSV to BVH Converter
Converts motion capture CSV files to BVH format for Blender import

Key features:
- Reads from CSV files instead of TXT
- Supports various CSV delimiters (comma, semicolon, tab)
- Handles headers automatically
- Maintains compatibility with existing BVH output format
...
class CSVBVHConverter:
    """Converts motion capture CSV data to BVH format"""
    ...
    def load_csv_file(self, filename: str) -> List[List[float]]:
        ...
        with open(filename, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if first_row:
                try:
                    ...
                    if self.debug_mode:
                        print(f"Skipping header row with {len(first_row)} columns")
    ...
    def load_mocap_data(self, joint_centers_file: str, joint_rotations_file: str):
        ...
        # Load joint centers
        joint_centers = self.load_csv_file(joint_centers_file)
        print(f"Loaded {len(joint_centers)} frames of joint center data")
        
        # Load joint rotations  
        joint_rotations = self.load_csv_file(joint_rotations_file)
...
def main():
    """Main function to convert CSV mocap data to BVH"""
    debug_mode = "--debug" in sys.argv
    
    if len(sys.argv) < 2 or (len(sys.argv) == 2 and sys.argv[1] == "--debug"):
        print("Usage: python mocap_to_bvh_csv.py [output_filename.bvh] [--debug]")
        print("Will use default CSV files: jointcenterscooper.csv, jointrotationscooper.csv")
        output_file = "cooper_baseball_motion.bvh"
    else:
        output_file = sys.argv[1] if sys.argv[1] != "--debug" else "cooper_baseball_motion.bvh"
    
    # Check if CSV files exist, if not, try TXT files
    joint_centers_file = "jointcenterscooper.csv"
    joint_rotations_file = "jointrotationscooper.csv"
    
    # If CSV files don't exist, print helpful message
    if not os.path.exists(joint_centers_file) or not os.path.exists(joint_rotations_file):
        print(f"CSV files not found. Looking for TXT files to convert...")
        
        # Check if TXT files exist
        txt_centers = "jointcenterscooper.txt"
        txt_rotations = "jointrotationscooper.txt"
        
        if os.path.exists(txt_centers) and os.path.exists(txt_rotations):
            print(f"\nFound TXT files. Please run the following command to convert them to CSV:")
            print(f"  python txt_to_csv_converter.py --convert-all")
            print(f"\nThen run this script again.")
            return
        else:
            print(f"Error: No motion capture files found (neither CSV nor TXT)")
            return
    
    # Create converter and process data
    converter = CSVBVHConverter(debug_mode=debug_mode)
